...
exports.generateWeeklyRecommendations = exports.generateRecommendations = void 0;
...
const generateRecommendations = async (userId, limit = 10) => {
    ...
    const userVotes = await Vote_1.Vote.find({ userId }).populate('movieId');
    const likedMovies = userVotes.filter((v) => v.voteType === 'like').map((v) => v.movieId);
    // Get user's reviews
    const userReviews = await MovieHistory_1.MovieHistory.find({
    ...
    for (const movie of allMovies) {
        ...
        const similarUserVotes = await Vote_1.Vote.find({
        ...
        const hasNegativePattern = negativeKeywords.some((keyword) => movieDescription.includes(keyword));
        if (hasNegativePattern) {
            score -= 20;
            reasons.push('May contain elements you disliked');
        }
        // Avoid already watched movies
        const alreadyWatched = userReviews.some((review) => review.movieIds && review.movieIds.some((m) => m.toString() === movie._id.toString()));
        if (alreadyWatched) {
...
const generateWeeklyRecommendations = async () => {
    ...
    for (const user of users) {
        try {
            const recommendations = await (0, exports.generateRecommendations)(user._id.toString(), 5);
...
exports.generateWeeklyRecommendations = generateWeeklyRecommendations;
